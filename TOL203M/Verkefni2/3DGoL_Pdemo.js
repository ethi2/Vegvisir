...
const sq3 = 1/Math.sqrt(3);//mögulega draga padding frá e-n veginn?
...
function initialSeeding(iO)
{
	...
	console.log(`${gen1count} initial living cells`);
}
initialSeeding(initialOdds);
...
let worldScaler = mat4();
//worldScaler = mult(translate(0.0,0.0,-2.0),worldScaler);
//worldScaler = mult(perspective(75.0,1.0,1.0,3.0),worldScaler);//fovy,aspect,near,far;cube-persp defaults: 65.0,1.0,0.3,3.0
worldScaler = mult(scalem(sq3,sq3,sq3),worldScaler);
//worldScaler = lookAt( vec3(2.5,0.0,1.5), vec3(0.0,0.0,0.0), vec3(0.0,0.0,1.0) );
function render()
{
...
function manualstep(Dn)
{
	...
	Dn.nextSibling.innerText = logStr;
	console.log(logStr);
}
render();
//----
const viewSettings = [document.getElementById("cb0").checked,document.getElementById("cb1").checked,document.getElementById("cb2").checked];
function viewChanger(X)
{
	viewSettings[X] = !viewSettings[X];
	let temp = mat4();
	if(viewSettings[0]){temp = mult(translate(0.0,0.0,-2.0),temp);}
	if(viewSettings[1]){temp = mult(perspective(75.0,1.0,1.0,3.0),temp);}
	if(viewSettings[2]){temp = mult(scalem(sq3,sq3,sq3),temp);}
	worldScaler = temp;
}
